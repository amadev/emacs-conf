...
Или `M-x amadev/tangle-and-reload-init`.

#+BEGIN_SRC elisp :tangle ~/.emacs.d/init.el
(defvar amadev/emacs-conf-org-file
  (expand-file-name "~/emacs-conf/emacs.org")
  "Org file that tangles into `user-init-file'.")

(defun amadev/tangle-and-reload-init ()
  "Tangle `amadev/emacs-conf-org-file' and reload `user-init-file'."
  (interactive)
  (require 'org)
  (require 'ob-tangle)
  (let ((org-file amadev/emacs-conf-org-file)
        (init-file (or user-init-file (expand-file-name "~/.emacs.d/init.el")))
        (org-confirm-babel-evaluate nil))
    (unless (file-exists-p org-file)
      (user-error "Org config not found: %s" org-file))
    ...
    (message "Tangled %s and reloaded %s" org-file init-file)))
#+END_SRC
...
(use-package magit
  :init
  ;; Magit ships with :no-byte-compile: t; stale .elc files break
  ;; transient prefixes (e.g. void-variable magit-log).
  (let ((elpa (expand-file-name "elpa" user-emacs-directory)))
    (when (file-directory-p elpa)
      (dolist (pkg (directory-files elpa t "^magit-"))
        (dolist (elc (directory-files pkg t "\\.elc\\'"))
          (delete-file elc))))))
#+END_SRC
...
(defun grep-at-point ()
  ...
  (let ((s (thing-at-point 'symbol t)))
    (ack (concat "ag -U -i --nogroup --nocolor " s " --ignore tests")
...
To use virtualenv from poetry:
poetry config virtualenvs.in-project true
# cat /home/amadev/.config/pypoetry/config.toml
poetry install
...
(defvar comint-history-enabled nil
  "Wether log comint history or not.")
#+END_SRC
...
(defun comint-write-input-ring-before-output (str)
  (if comint-history-enabled (if str (comint-write-input-ring)))
  str)

(if comint-history-enabled (add-hook
...
*** Shell interrupt: optional kill-style interrupt

By default =comint-interrupt-subjob= (=C-c C-c=) may send the tty =VINTR=
character (=^C=) down the PTY instead of calling =kill(2)=.  =C-u C-c C-c=
uses the foreground process group from =process-attributes= (=tpgid=) and
=signal-process= with =SIGINT= (=kill -INT= to a negative process group).
=C-u C-u C-c C-c= uses =SIGKILL= (=kill -9= style).

#+BEGIN_SRC elisp :tangle ~/.emacs.d/init.el
(require 'helm)
(require 'comint)

(defun shell-mode--signal-foreground-pgrp (proc sig)
  "Send SIG (symbol, e.g. SIGINT or SIGKILL) to the foreground tty pgrp of PROC.
If the tpgid attribute is missing, signal PROC itself."
  (let* ((pid (process-id proc))
         (attrs (and pid (process-attributes pid)))
         (tpgid (and attrs (alist-get 'tpgid attrs))))
    (if (and tpgid (numberp tpgid) (/= (truncate tpgid) 0))
        (signal-process (- (truncate tpgid)) sig)
      (signal-process proc sig))))

(defun shell-mode-interrupt-subjob-via-signal (signal)
  "Skip pending comint input, then send SIGNAL via `signal-process' / foreground pgrp.
SIGNAL is a symbol such as SIGINT or SIGKILL (not the PTY ^C path)."
  (comint-skip-input)
  (when-let ((proc (get-buffer-process (current-buffer))))
    (shell-mode--signal-foreground-pgrp proc signal)))

(defun shell-mode-interrupt-subjob-dwim (arg)
  "Interrupt subjob: no prefix is default comint; C-u is kill-style SIGINT; C-u C-u is SIGKILL."
  (interactive "P" comint-mode)
  (cond ((null arg)
         (comint-interrupt-subjob))
        ((equal arg '(16))
         (shell-mode-interrupt-subjob-via-signal 'SIGKILL))
        (t
         (shell-mode-interrupt-subjob-via-signal 'SIGINT))))

(defun shell-mode-delete-above-current-line ()
  "Delete all text from the start of the buffer through the line above point."
...
(defun shell-mode-custom-keys ()
  ...
  (local-set-key (kbd "C-c C-c") #'shell-mode-interrupt-subjob-dwim)
  (local-set-key (kbd "C-c u") #'shell-mode-delete-above-current-line))
...
(if comint-history-enabled (progn (add-hook 'comint-input-filter-functions 'comint-log-input)
...
;; (key-chord-define-global "ff" 'agent-shell)
(key-chord-define-global "kk" 'agent-shell-cursor-start-agent)
#+END_SRC
...
(use-package ack
  ...
  :init (setq
         ...
         ack-command "ag -U -i --nogroup --nocolor ")
  :config (defalias 'grep 'ack))

(setq grep-command "ag -U -i --nogroup --nocolor ")
...
(defun cm-display-compilation-buffer (buffer alist)
  "Show compilation BUFFER, reusing a visible compilation window if any."
  (or (display-buffer-reuse-mode-window buffer alist)
      (display-buffer-at-bottom buffer alist)))

(add-to-list 'display-buffer-alist
             '("\\`\\*compilation"
               (cm-display-compilation-buffer)
               (mode . compilation-mode)
               (reusable-frames . visible)
               (inhibit-same-window . t)))

(defun cm-open-last-buffer ()
  "Switch to the last compilation buffer."
  (interactive)
  (switch-to-buffer
   (or (and next-error-last-buffer
            (buffer-live-p next-error-last-buffer)
            (compilation-buffer-p next-error-last-buffer)
            next-error-last-buffer)
       (compilation-find-buffer t))))

(defun cm-kill-all-buffers ()
  "Kill all compilation buffers, stopping any running processes."
  (interactive)
  (let ((killed 0))
    ...
    (message "Killed %d compilation buffer(s)" killed)))

(global-set-key (kbd "<f10>") 'cm-open-last-buffer)
#+END_SRC
...
(add-hook 'python-mode-hook 'helm-xref-configure)
...
(setq gptel-default-mode 'org-mode
      ...
      gptel-track-media t
      gptel--debug t
      gptel-api-key (get-pass "claude-api-key"))
;; defer nothing
;; (require 'gptel)
;; (require 'gptel-curl)
;; (require 'gptel-transient)

(setq backend-claude (gptel-make-anthropic "claude" :stream t :key gptel-api-key))

(setq backend-ollama-local (gptel-make-ollama "ollama-local" :host "localhost:11434" :stream t :models '("qwen3.5:9b" "qwen3:latest" "llama3.2:1b" "mistral:latest" "gemma3:4b" "TinyLlama:latest")))

;; run owui-proxy.sh ; curl 192.168.1.5:11434
(setq backend-ollama-forcode (gptel-make-ollama "ollama-forcode" :host "192.168.1.5:11434" :stream t :models '("command-a:111b" "llama3.1:latest" "gpt-oss:20b" "gpt-oss:120b" "wizardlm2:8x22b" "phi4:14b" "wizardlm2:7b" "deepseek-coder-v2:236b" "qwen3:235b" "codellama:70b" "deepseek-r1:70b" "deepseek-r1:671b" "evilfreelancer/enbeddrus:v0.2" "magistral:24b" "qwen3:30b")))

;; (setq owuill (gptel-make-ollama "owuill"
;;    :host "localhost:11435"
;;    :stream t
;;    ))

;; (setq gptel-backend owui
;;       gptel-model 'codellama:70b)
...
(setq gptel-directives (gjg/gptel-build-directives "~/s/AIPIHKAL/system-prompts/"))
#+END_SRC
...
(use-package agent-shell
  ...
  :config
  (defun amadev/agent-shell--project-key (project-root)
    "Return a filesystem-safe key for PROJECT-ROOT."
    (let ((cwd (string-remove-suffix "/" (expand-file-name project-root))))
      (replace-regexp-in-string "/" "-" (string-remove-prefix "/" cwd))))

  (defun amadev/agent-shell--emacs-d-project-dir (project-root)
    "Return ~/.emacs.d/agent-shell/<project-key>/ for PROJECT-ROOT."
    (locate-user-emacs-file
     (concat "agent-shell/" (amadev/agent-shell--project-key project-root))))

  (defun amadev/agent-shell-dot-subdir (subdir)
    "Store agent-shell project data under ~/.emacs.d/agent-shell/."
    (expand-file-name subdir
                       (amadev/agent-shell--emacs-d-project-dir
                        (agent-shell-cwd))))

  (setopt agent-shell-dot-subdir-function #'amadev/agent-shell-dot-subdir)

  (declare-function agent-shell--tool-call-command-to-string "agent-shell" (command))
  ...
  (declare-function shell-maker-set-buffer-name "shell-maker" (buffer new-name))

  (defvar-local amadev/agent-shell--renamed-from-prompt-p nil)

  (defun amadev/agent-shell--prompt-snippet (title &optional max-words max-len)
    "Return first MAX-WORDS words of TITLE, capped at MAX-LEN chars."
    (let* ((words (or max-words 5))
           (len (or max-len 60))
           (line (string-trim (replace-regexp-in-string "[\r\n]+" " " title)))
           (snippet (mapconcat #'identity
                               (seq-take (split-string line "\\s-+" t) words)
                               " ")))
      (if (> (length snippet) len)
          (concat (substring snippet 0 (- len 3)) "...")
        snippet)))

  (defun amadev/agent-shell--rename-buffer-with-title (title)
    "Append a TITLE snippet to the agent-shell buffer name, once per buffer."
    (when (and (stringp title) (not (string-empty-p title))
               (not amadev/agent-shell--renamed-from-prompt-p))
      (let* ((state agent-shell--state)
             (agent-name (or (map-nested-elt state '(:agent-config :mode-line-name))
                             (map-nested-elt state '(:agent-config :buffer-name))
                             "Agent"))
             (base (agent-shell--format-buffer-name
                    agent-name (agent-shell--project-name)))
             (snippet (amadev/agent-shell--prompt-snippet title))
             (new-name (format "%s [%s]" base snippet)))
        (shell-maker-set-buffer-name (current-buffer) new-name)
        (setq amadev/agent-shell--renamed-from-prompt-p t))))

  (defun amadev/agent-shell-rename-from-prompt (event)
    (when (eq (map-elt event :event) 'session-title-changed)
      (amadev/agent-shell--rename-buffer-with-title
       (map-elt (map-elt event :data) :title))))

  (defun amadev/agent-shell-on-session-selected (_event)
    (amadev/agent-shell--rename-buffer-with-title
     (map-nested-elt agent-shell--state '(:session :title))))

  (defun amadev/agent-shell-setup-prompt-buffer-name ()
    (setq amadev/agent-shell--renamed-from-prompt-p nil)
    (agent-shell-subscribe-to
     :shell-buffer (current-buffer)
     :event 'session-title-changed
     :on-event #'amadev/agent-shell-rename-from-prompt)
    (agent-shell-subscribe-to
     :shell-buffer (current-buffer)
     :event 'session-selected
     :on-event #'amadev/agent-shell-on-session-selected))

  (add-hook 'agent-shell-mode-hook #'amadev/agent-shell-setup-prompt-buffer-name)

  (defun agent-shell-desktop-notify--truncate (s maxlen)
    (if (and s (> (length s) maxlen))
        (concat (substring s 0 (- maxlen 3)) "...")
      s))

  (defun agent-shell-desktop-notify--permission-body (tool-call)
    "Build desktop notification body describing TOOL-CALL."
    (if (null tool-call)
        "Open the agent shell buffer to review and respond."
      (let* ((title (map-elt tool-call :title))
             (kind (map-elt tool-call :kind))
             (cmd (ignore-errors
                    (agent-shell--tool-call-command-to-string
                     (map-nested-elt tool-call '(rawInput command)))))
             (path (or (map-nested-elt tool-call '(rawInput filepath))
                       (map-nested-elt tool-call '(rawInput fileName))
                       (map-nested-elt tool-call '(rawInput path))
                       (map-nested-elt tool-call '(rawInput file_path)))))
        (let ((body (string-trim
                     (concat (when title (concat title "\n"))
                             (when kind (concat "Kind: " kind "\n"))
                             (when (and cmd (not (string-empty-p cmd)))
                               (concat "Command: " cmd "\n"))
                             (when path (concat "Path: " path "\n"))))))
          (if (string-empty-p body)
              "Permission requested for a tool call."
            body)))))

  (defun agent-shell-desktop-notify--send (tool-call)
    (when (require 'notifications nil t)
      (let ((body (agent-shell-desktop-notify--truncate
                   (agent-shell-desktop-notify--permission-body tool-call)
                   800)))
        (condition-case nil
            (notifications-notify
             :title "Agent shell: permission required"
             :body body
             :app-name "Emacs"
             :urgency 'normal)
          (error nil)))))

  (defun agent-shell-desktop-notify-on-permission (event)
    (agent-shell-desktop-notify--send (map-elt (map-elt event :data) :tool-call)))

  (defun agent-shell-desktop-notify-setup ()
    (agent-shell-subscribe-to
     :shell-buffer (current-buffer)
     :event 'permission-request
     :on-event #'agent-shell-desktop-notify-on-permission))

  (add-hook 'agent-shell-mode-hook #'agent-shell-desktop-notify-setup))
...
(use-package agent-recall
  ...
  (require 'cl-lib)
  :config
  (setq agent-recall-search-paths '("~/s" "~/bin"))

  (defun amadev/agent-recall--emacs-d-transcript-root ()
    "Return ~/.emacs.d/agent-shell/."
    (file-name-as-directory
     (expand-file-name "agent-shell" user-emacs-directory)))

  (defun amadev/agent-recall--emacs-d-transcript-path-p (dir)
    "Return non-nil when DIR looks like a central agent-shell transcripts path."
    (let* ((root (amadev/agent-recall--emacs-d-transcript-root))
           (dir (file-name-as-directory (expand-file-name dir)))
           (rel (when (string-prefix-p root dir)
                  (substring dir (length root)))))
      (and rel
           (string-match-p "^[^/]+/transcripts/?\\'" rel)
           (not (string-match "/\\.agent-shell/" rel)))))

  (defun amadev/agent-recall--emacs-d-transcript-dir-p (dir)
    "Return non-nil when DIR is an existing central transcripts directory."
    (and (amadev/agent-recall--emacs-d-transcript-path-p dir)
         (file-directory-p dir)))

  (defun amadev/agent-recall--emacs-d-project-key (transcript-dir)
    (when (string-match "/agent-shell/\\([^/]+\\)/transcripts"
                        (file-name-as-directory transcript-dir))
      (match-string 1 transcript-dir)))

  (defun amadev/agent-recall--project-root-from-transcript-dir (transcript-dir)
    "Read project root from a transcript header in TRANSCRIPT-DIR."
    (when (file-directory-p transcript-dir)
      (cl-some (lambda (file)
                 (agent-recall--read-working-directory file))
               (directory-files transcript-dir t "\\.md\\'" t))))

  (defun amadev/agent-recall--key-mask->path (parts mask)
    (let ((result (car parts)))
      (dotimes (i (1- (length parts)))
        (setq result
              (concat result
                      (if (zerop (logand (lsh 1 i) mask)) "-" "/")
                      (nth (1+ i) parts))))
      result))

  (defun amadev/agent-recall--project-root-from-key (key)
    "Best-effort reverse of `amadev/agent-shell--project-key'."
    (let* ((parts (split-string key "-" t))
           (gaps (max 0 (1- (length parts))))
           (best nil))
      (when parts
        (cl-loop for mask from 0 below (ash 1 gaps)
                 for candidate = (amadev/agent-recall--key-mask->path parts mask)
                 when (file-directory-p (expand-file-name candidate "/"))
                 do (when (or (null best) (> (length candidate) (length best)))
                      (setq best candidate))))
      (and best (expand-file-name best "/"))))

  (defun amadev/agent-recall--project-root-for-transcript-dir (transcript-dir)
    (or (amadev/agent-recall--project-root-from-transcript-dir transcript-dir)
        (amadev/agent-recall--project-root-from-key
         (amadev/agent-recall--emacs-d-project-key transcript-dir))))

  (defun amadev/agent-recall--project-name-for-transcript-dir (transcript-dir)
    (file-name-nondirectory
     (directory-file-name
      (or (amadev/agent-recall--project-root-for-transcript-dir transcript-dir)
          (amadev/agent-recall--emacs-d-project-key transcript-dir)))))

  (defun amadev/agent-recall--emacs-d-transcript-dirs ()
    (let (dirs)
      (when (file-directory-p (amadev/agent-recall--emacs-d-transcript-root))
        (dolist (entry (directory-files (amadev/agent-recall--emacs-d-transcript-root)
                                         t "^[^.]"))
          (when (file-directory-p entry)
            (let ((transcripts (expand-file-name "transcripts" entry)))
              (when (amadev/agent-recall--emacs-d-transcript-dir-p transcripts)
                (push transcripts dirs))))))
      dirs))

  (defun amadev/agent-recall--discover-transcript-dirs ()
    (let (dirs)
      ...
      (setq dirs (append dirs (amadev/agent-recall--emacs-d-transcript-dirs)))
      (seq-filter #'file-directory-p dirs)))

  (defun amadev/agent-recall-reindex ()
    "Rebuild transcript index including ~/.emacs.d/agent-shell/ storage."
    (interactive)
    (unless agent-recall-search-paths
      (user-error "`agent-recall-search-paths' is not set"))
    (let* ((dirs (delete-dups (amadev/agent-recall--discover-transcript-dirs)))
           (new-index (make-hash-table :test 'equal))
           (file-count 0)
           (project-count (length dirs))
           (without-session 0))
      (dolist (dir dirs)
      ...
      (message "agent-recall: indexed %d transcripts across %d projects%s"
               ...
               (if (> without-session 0)
                   (format " (%d without session IDs -- run M-x agent-recall-backfill)"
                           without-session)
                 ""))))

  (defun amadev/agent-recall--project-name-advice (orig dir)
    (if (amadev/agent-recall--emacs-d-transcript-dir-p dir)
        (amadev/agent-recall--project-name-for-transcript-dir dir)
      (funcall orig dir)))

  (defun amadev/agent-recall--project-root-advice (orig dir)
    (if (amadev/agent-recall--emacs-d-transcript-dir-p dir)
        (or (amadev/agent-recall--project-root-for-transcript-dir dir)
            (file-name-directory
             (directory-file-name
              (amadev/agent-recall--emacs-d-transcript-root))))
      (funcall orig dir)))

  (defun amadev/agent-recall--transcript-file-p-advice (orig file)
    (or (funcall orig file)
        (and file
             (amadev/agent-recall--emacs-d-transcript-path-p
              (file-name-directory file)))))

  (defun amadev/agent-recall--index-dirs-advice (orig)
    (seq-filter #'file-directory-p (funcall orig)))

  (advice-add #'agent-recall--project-name :around
              #'amadev/agent-recall--project-name-advice)
  (advice-add #'agent-recall--project-root :around
              #'amadev/agent-recall--project-root-advice)
  (advice-add #'agent-recall--transcript-file-p :around
              #'amadev/agent-recall--transcript-file-p-advice)
  (advice-add #'agent-recall--index-dirs :around
              #'amadev/agent-recall--index-dirs-advice)
  (advice-add #'agent-recall-reindex :override #'amadev/agent-recall-reindex)

  ;; Cursor ACP session backfill (~/.cursor/acp-sessions/)
  (defcustom amadev/agent-recall-cursor-acp-sessions-dir
    (expand-file-name "acp-sessions" (expand-file-name ".cursor" "~"))
    "Directory where `cursor-agent acp' stores session data."
    :type 'directory
    :group 'agent-recall)

  (defconst amadev/agent-recall--cursor-message-row-script
    "import sqlite3,sys
store,needle=sys.argv[1],sys.argv[2]
prefix=' '.join(needle.lower().split())[:80]
if not prefix: sys.exit(1)
conn=sqlite3.connect(store)
for rowid,(data,) in enumerate(conn.execute('SELECT data FROM blobs ORDER BY rowid')):
    text=' '.join(bytes(data).decode('utf-8','ignore').lower().split())
    if prefix in text:
        print(rowid); sys.exit(0)
sys.exit(1)")

  (defun amadev/agent-recall--cursor-message-row (session-dir transcript-msg)
    "Return the earliest blob row index containing TRANSCRIPT-MSG, or nil."
    (let ((store (expand-file-name "store.db" session-dir)))
      (when (and (file-exists-p store) transcript-msg)
        (let ((output (string-trim
                        (shell-command-to-string
                         (format "python3 -c %S %S %S"
                                 amadev/agent-recall--cursor-message-row-script
                                 store
                                 transcript-msg)))))
  ...
  (defun amadev/agent-recall--cursor-session-entries (project-root)
    "Return Cursor ACP sessions for PROJECT-ROOT as (ID . SESSION-DIR) alist."
    (let ((root amadev/agent-recall-cursor-acp-sessions-dir)
          (project (amadev/agent-recall--normalize-path project-root))
          entries)
      (when (file-directory-p root)
        (dolist (meta (directory-files root t "meta\\.json\\'"))
          (let* ((session-dir (file-name-as-directory (file-name-directory meta)))
                 (session-id (file-name-nondirectory
                              (directory-file-name session-dir)))
                 (meta-data (with-temp-buffer
                              (insert-file-contents meta)
                              (json-read)))
                 (cwd (cdr (assoc 'cwd meta-data))))
            (when (and cwd
                       (string= project (amadev/agent-recall--normalize-path cwd))
                       (file-exists-p (expand-file-name "store.db" session-dir)))
              (push (cons session-id session-dir) entries)))))
      entries))

  (defun amadev/agent-recall--cursor-agent-p (file)
    "Return non-nil when FILE is a Cursor agent-shell transcript."
    (equal (agent-recall--read-agent-name file) "Cursor"))

  (defun amadev/agent-recall--normalize-path (path)
    (file-name-as-directory (expand-file-name (or path ""))))

  (defun amadev/agent-recall--match-cursor-session-id (file)
    "Match transcript FILE to a Cursor ACP session ID, or nil."
    (when (amadev/agent-recall--cursor-agent-p file)
      (let* ((transcript-dir (agent-recall--transcript-dir-from-file file))
             (project-root (agent-recall--project-root transcript-dir))
             (transcript-msg (agent-recall--transcript-first-message file))
             (sessions (amadev/agent-recall--cursor-session-entries project-root))
             (best-id nil)
             (best-row most-positive-fixnum))
        (when (and transcript-msg sessions)
          (dolist (entry sessions)
            (let* ((session-id (car entry))
                   (session-dir (cdr entry))
                   (row (amadev/agent-recall--cursor-message-row
                         session-dir transcript-msg)))
              (when (and row (< row best-row))
                (setq best-id session-id
                      best-row row))))
          best-id))))

  (defun amadev/agent-recall--resolve-session-id-cursor-advice (orig file)
    (let* ((embedded (agent-recall--read-embedded-session-id file))
           (cached (gethash file agent-recall--session-id-cache)))
      (cond
       (embedded embedded)
       ((and cached (not (eq cached 'none))) cached)
       (t
        (let ((session-id
               (or (unless (eq cached 'none) (funcall orig file))
                   (amadev/agent-recall--match-cursor-session-id file))))
          (puthash file (or session-id 'none) agent-recall--session-id-cache)
          session-id)))))

  (defun amadev/agent-recall-backfill-cursor-pass (&optional write-mode)
    "Match Cursor transcripts and optionally write `**Session:**' headers."
      ...
      (maphash
       (lambda (file entry)
         (when (and (file-exists-p file)
                    (not (agent-recall--read-embedded-session-id file))
                    (amadev/agent-recall--cursor-agent-p file))
           (let ((project (plist-get entry :project))
                 (session-id (amadev/agent-recall--match-cursor-session-id file)))
             (if session-id
                 (progn
                   (cl-incf matched)
                   (when write-mode
                     (agent-recall--write-session-id-to-file file session-id)
                     (agent-recall--index-add file session-id)
                     (puthash file session-id agent-recall--session-id-cache)
                     (push file modified-files))
                   (when (get-buffer "*agent-recall-backfill*")
                     (with-current-buffer "*agent-recall-backfill*"
                       (goto-char (point-max))
                       (insert (format "  CURSOR:   [%s] %s → %s\n"
                                       project
               ...
               (when (get-buffer "*agent-recall-backfill*")
                 (with-current-buffer "*agent-recall-backfill*"
                   (goto-char (point-max))
                   (insert (format "  CURSOR:   [%s] %s (no Cursor session match)\n"
                                   project
                                   (file-name-nondirectory file)))))))))
       agent-recall--index)
      (when (and write-mode modified-files)
        (agent-recall--index-save))
      (when (> matched 0)
        (message "agent-recall cursor backfill: matched %d, no match %d"
                 matched no-match))
      (list matched no-match modified-files)))

  (defun amadev/agent-recall-backfill-cursor (&optional write-mode)
    "Backfill session IDs for Cursor transcripts from ~/.cursor/acp-sessions/."
    (interactive "P")
    (amadev/agent-recall-backfill-cursor-pass write-mode))

  (defun amadev/agent-recall-backfill-around (orig &optional write-mode)
    (funcall orig write-mode)
    (amadev/agent-recall-backfill-cursor-pass write-mode))

  (advice-add #'agent-recall--resolve-session-id :around
              #'amadev/agent-recall--resolve-session-id-cursor-advice)
  (advice-add #'agent-recall-backfill :around
              #'amadev/agent-recall-backfill-around)

  (defcustom amadev/agent-recall-browse-prefer-helm t
    "When non-nil, `agent-recall-browse' uses Helm with live preview."
    :type 'boolean
    :group 'agent-recall)

  (defvar amadev/agent-recall--helm-temporary-buffers nil
    "Temporary transcript buffers opened during Helm browse preview.")

  (defun amadev/agent-recall--browse-helm-teardown-hooks ()
    (remove-hook 'helm-after-update-hook
                 #'amadev/agent-recall--browse-helm-update-preview t)
    (remove-hook 'helm-move-selection-after-hook
                 #'amadev/agent-recall--browse-helm-update-preview t))

  (defun amadev/agent-recall--browse-helm-cleanup ()
    (amadev/agent-recall--browse-helm-teardown-hooks)
    (mapc (lambda (buf)
            (when (buffer-live-p buf)
              (kill-buffer buf)))
          amadev/agent-recall--helm-temporary-buffers)
    (setq amadev/agent-recall--helm-temporary-buffers nil))

  (defun amadev/agent-recall--browse-helm-buffer (file)
    "Return a buffer visiting transcript FILE, creating a temp one if needed."
    (or (get-file-buffer file)
        (let ((buf (find-file-noselect file)))
          (push buf amadev/agent-recall--helm-temporary-buffers)
          buf)))

  (defun amadev/agent-recall--browse-helm-show-preview (file)
    (when-let* ((file (and file (stringp file)))
                (buf (amadev/agent-recall--browse-helm-buffer file)))
      (with-helm-window
        (with-selected-window (helm-persistent-action-display-window :split t)
          (switch-to-buffer buf 'norecord)
          (goto-char (point-min))
          (when agent-recall-auto-transcript-mode
            (agent-recall-transcript-mode 1))))))

  (defun amadev/agent-recall--browse-helm-update-preview ()
    (condition-case nil
        (unless (or (not (bound-and-true-p helm-alive-p))
                    (helm-empty-buffer-p)
                    (helm-pos-header-line-p))
          (amadev/agent-recall--browse-helm-show-preview
           (helm-get-selection nil nil)))
      (error nil)))

  (defun amadev/agent-recall--browse-helm-persistent-action (file)
    (amadev/agent-recall--browse-helm-show-preview file))

  (defun amadev/agent-recall--browse-helm-candidates (transcripts)
    "Build Helm (DISPLAY . FILE) candidates from TRANSCRIPTS."
    (mapcar
     (lambda (entry)
       (let* ((file (cdr entry))
              (display (car entry))
              (preview (when-let* ((idx-entry (gethash file agent-recall--index))
                                  (p (plist-get idx-entry :preview)))
                         (unless (string-empty-p p) p))))
         (cons (if preview (concat display (concat "  " preview)) display)
               file)))
     transcripts))

  (defun amadev/agent-recall--browse-helm (candidates)
    "Browse transcripts with Helm and live preview.
Returns the selected transcript file path, or nil."
    (require 'helm-core)
    (let ((default (car agent-recall--browse-history)))
      (unwind-protect
          (progn
            (add-hook 'helm-after-update-hook
                      #'amadev/agent-recall--browse-helm-update-preview nil t)
            (add-hook 'helm-move-selection-after-hook
                      #'amadev/agent-recall--browse-helm-update-preview nil t)
            (helm
             :sources
             (helm-build-sync-source "Transcripts"
               :candidates candidates
               :persistent-action #'amadev/agent-recall--browse-helm-persistent-action
               :persistent-help "Preview transcript"
               :action #'identity
               :nomark t
               :group 'agent-recall)
             :prompt "Transcript: "
             :buffer "*helm agent-recall*"
             :history 'agent-recall--browse-history
             :default default
             :preselect default))
        (amadev/agent-recall--browse-helm-cleanup))))

  (defun amadev/agent-recall-browse-helm-advice (orig)
    (interactive)
    (if (and agent-recall-browse-preview
             amadev/agent-recall-browse-prefer-helm
        ...
        (progn
          ...
          (let* ((transcripts (agent-recall--list-transcripts)))
            ...
            (when-let ((file (amadev/agent-recall--browse-helm
                              (amadev/agent-recall--browse-helm-candidates
                               transcripts))))
              (agent-recall--open-transcript file))))
      (funcall orig)))

  (advice-add #'agent-recall-browse :around #'amadev/agent-recall-browse-helm-advice)
...
(setq compilation-buffer-name-function
      (lambda (_mode) (generate-new-buffer-name "*compilation* ")))
(defun colorize-compilation-buffer ()
  (let ((inhibit-read-only t))
    (ansi-color-apply-on-region (point-min) (point-max))))
(add-hook 'compilation-filter-hook 'colorize-compilation-buffer)

(defun compilation-desktop-notify--command (buffer)
  "Return the shell command for this compilation buffer, or an empty string."
  (with-current-buffer buffer
    (or (and (boundp 'compilation-arguments)
             compilation-arguments
             (let ((a (car compilation-arguments)))
               (if (stringp a) a (prin1-to-string a))))
        (and (boundp 'compile-command) compile-command
             (if (stringp compile-command)
                 compile-command
               (prin1-to-string compile-command)))
        "")))

(defun compilation-desktop-notify (buffer finish-msg)
  "Desktop notification when compilation finishes (requires D-Bus / libnotify)."
  (when (and (buffer-live-p buffer)
             (require 'notifications nil t))
    (let* ((name (buffer-name buffer))
           (cmd (compilation-desktop-notify--command buffer))
           (failed (string-match-p "exited abnormally" finish-msg))
           (body (if (string-empty-p cmd)
                     (format "%s\n%s" name (string-trim finish-msg))
                   (format "%s\nCommand: %s\n%s" name cmd (string-trim finish-msg)))))
      (condition-case nil
          (notifications-notify
           :title (if failed "Compilation failed" "Compilation finished")
           :body body
           :app-name "Emacs"
           :urgency (if failed 'critical 'normal))
        (error nil)))))
(add-hook 'compilation-finish-functions #'compilation-desktop-notify)
