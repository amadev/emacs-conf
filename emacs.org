...
#+BEGIN_SRC elisp
  ...
  (defun my-erc-tracked-channels ()
    (let ((excluded '("#mos-nova" "#mos-nova-flood-ru")))
      (remove-if #'(lambda (row) (member row excluded)) (my-erc-joined-channels))))

  ;; (setq erc-track-priority-faces-only (my-erc-tracked-channels))
#+END_SRC

** terminal

Для работы с терминалом вполне хватает shell + rename buffer.
Если нужно запустить новый, использовать (start-shell <shell_name> <initial_cmd).

Проблема с tab completion очень элегантно решается с помощью /proc.
...
(defun start-shells ()
  (interactive)
  (start-shell "shell_default" "cd")
  (start-shell "shell_nova" "nact")
  (start-shell "shell_mos-con1" "ssh mos-con1")
  (start-shell "shell_mos-fuel" "ssh mos-fuel"))
#+END_SRC
...
(defun sh-switch-to-process-buffer ()
  (interactive)
  (pxop-to-buffer (process-buffer (get-process "shell")) t))
...
#+BEGIN_SRC elisp
  (defun run-test (arg)
    (interactive "P")
    (compile (concat "ltests.sh" " " (what-function arg)))
    (compile "rtests.sh"))
