...
(setq ack-command "ag -i --nogroup --nocolor ")
...
(define-minor-mode sh-send-mode
  ...
  :keymap (let ((map (make-sparse-keymap)))
            ...
            (define-key map (kbd "C-u C-c C-c") 'send-current-paragraph-or-region-and-step)
...
(setq org-agenda-files
      (mapcar
       ...
       '("task.org" "bookmark.org" "reference.org" "work-gcal.org" "book.org" "film.org" "att-log.org")))
#+END_SRC
...
(defun refile-org-files ()
  (let ((files '("task.org" "reference.org" "bookmark.org" "book.org" "film.org")))
...
Done issuges go to the bottom of the list. Can view it if needed.
...
The mass of the sun is $M_{sun} = 1.989*10^{30} kg$.  The radius of
the sun is $R_{sun} = 6.96*10^8m$.
...
(use-package org-gcal
  :ensure t
  :config
  (setq org-gcal-client-id "971118572262-uo129oc9kbkmbm1eoeq5rna4ep2ge5dl.apps.googleusercontent.com"
        org-gcal-client-secret "5yv7pP68WPU_nvmQy0l4_oR8"
